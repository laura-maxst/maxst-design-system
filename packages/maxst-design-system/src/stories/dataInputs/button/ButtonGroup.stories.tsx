...
const meta: Meta<typeof ButtonGroup> = {
  ...
  argTypes: {
    children: {
      description: `여러 button을 component 형태로 지정합니다.\n
    "React.ReactNode"
        `,
      control: {
        type: null,
    ...
    buttonWidth: {
      description:
        '그룹내의 모든 버튼이 같은 width값으로 고정이 필요할 경우 `px`, `%`를 붙여 `string`으로 작성합니다. \nbutton의 `max-width`값(`330px`) 이상 지정할 수 없습니다.',
    },
    fullWidth: {
      description:
        '자식 button들의 width를 `100%`로 정의해주지만 button의 `max-width`값(`330px`) 이상 지정할 수 없습니다.',
    },
    direction: {
      description: '기본적으로 horizontal 방향입니다.',
    },
    align: {
